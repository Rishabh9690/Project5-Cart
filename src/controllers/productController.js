...
const getProduct = async function (req, res)
{
        try{
            ...
            for(let i=0;i<findName.length;i++)
            {
                ...
                if(newSize) arr.push(findName[i].title);
            }
          filter["title"]=name;
        }
    
        if(priceGreaterThan!=null && priceLessThan==null )filter["price"]={$gt:priceGreaterThan};
        
        if(priceGreaterThan==null && priceLessThan!=null )filter["price"]={$lt:priceLessThan};
        
        if(priceGreaterThan!=null && priceLessThan!=null ) filter["price"]={$gt:priceGreaterThan,$lt:priceLessThan};
        
        if(size) 
        {
            const searchSize = await productModel.find({availableSizes: size, isDeleted: false}).sort({price: sortPrice});
            if(searchSize.length !== 0) return res.status(200).send({ status: true, message: 'Success', data: searchSize});
            else return res.status(404).send({status: false, message: `product not found with this ${size}`});  
        }

        if(name) 
        {
            const searchName = await productModel.find({title: {$regex: name}, isDeleted: false}).sort({price: priceSort});
            if(searchName.length !== 0) return res.status(200).send({status: true, message: 'Success', data: searchName});
            else return res.status(404).send({status: false, message: `product not found with this ${name}`});
        ...
        if(sortPrice==1)
        {
           ...
           if(findPrice.length==0) return res.status(404).send({status:false,message:"data not found..!!"});
...
const getProductbyId = async function (req, res) 
{
    try
    {
        ...
        if(findProduct.isDeleted == true) return res.status(400).send({ status:false, msg: "product is deleted..!!" });
...
const updateProduct = async function (req, res) 
{
    try 
    {
        ...
        if (!isValidDetails(data)) return res.status(400).send({ status: false, msg: "please provide details to update..!!" });

        let { title, description, price, currencyId, currencyFormat, productImage,availableSizes } = data;
        const dataObject = {};
        if (isValid(title)) dataObject['title'] = title.trim();
        

        if (isValid(description)) dataObject['description'] = description.trim();
        
        if (isValid(price)) dataObject['price'] = price.trim();

        if (isValid(currencyId)) dataObject['currencyId'] = currencyId.trim();
        
        if (isValid(currencyFormat)) dataObject['currencyFormat'] = currencyFormat.trim();
        ...
        let updatedProduct = await productModel.findOneAndUpdate({_id:productId},
            ...
            if (!updatedProduct) return res.status(404).send({ status: false, msg: "user profile not found..!!" });
...
const deleteProduct = async function (req, res) 
{
    try
    {
        ...
        if (findProduct.isDeleted == true) return res.status(400).send({status:false, message:"product already deleted..!!"});
