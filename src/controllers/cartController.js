...
const createCart = async function (req, res)
{
  try 
  {
      ...
      {  
          if (!isValid(cartId)) return res.status(400).send({ status: false, message: "cartId could not be blank..!!" });
...
const getCart = async function (req, res) 
{
  try
  {
    ...
    if (!findCart) return res.status(400).send({ status: false, message: "no cart exist with this id..!!" });
    
    if(findCart.totalPrice === 0) return res.status(404).send({status:false, msg:"your cart is empty..!!"});
